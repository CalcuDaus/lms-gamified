    ...
    <div class="flex justify-center items-center font-balo max-w-[1400px] flex-col mx-auto p-6">
        ...
        <div class="w-full mb-8">
            <h1 class="text-4xl font-bold text-gray-900 dark:text-white mb-2">
                ...
                <p class="text-gray-600 dark:text-gray-400 text-lg">{{ __('messages.continue_journey') }}</p>
                </div>

                {{-- Stats Cards Grid - 1 column on mobile, 2 on tablet, 4 on desktop --}}
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 w-full mb-8">
                    {{-- XP Progress Card --}}
                    <div class="flex flex-col text-gray-600 dark:text-[#9b9b9b] hover:dark:text-[#d6d6d6] transition-all duration-300 shadow-custom rounded-3xl p-6"
                        ...
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <span
                                ...
                                {{ __('messages.level') }} {{ auth()->user()->level }}
                            </span>
                        </div>
                        <h3 class="text-lg font-semibold mb-2 text-gray-900 dark:text-white">{{ __('messages.total_xp') }}</h3>
                            ...
                            <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3">
                                @php
                                    $xpProgress = auth()->user()->next_level_xp > 0
                                        ...
                                        : 0;
                                @endphp
                                <div class="bg-linear-to-r from-purple-500 to-pink-500 h-3 rounded-full progress-bar-fill"
                    ...
                    <div class="flex flex-col text-gray-600 dark:text-[#9b9b9b] hover:dark:text-[#d6d6d6] transition-all duration-300 shadow-custom rounded-3xl p-6"
                        ...
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <span
                                class="text-sm font-semibold bg-orange-100 dark:bg-orange-900 text-orange-700 dark:text-orange-300 px-3 py-1 rounded-full">
                                {{ __('messages.daily') }}
                            </span>
                        </div>
                        <h3 class="text-lg font-semibold mb-2 text-gray-900 dark:text-white">{{ __('messages.current_streak') }}</h3>
                        <p class="text-5xl font-bold count-animation text-gray-900 dark:text-white">
                            {{ auth()->user()->current_streak ?? 0 }}</p>
                        <p class="text-sm mt-2">
                            @if(auth()->user()->current_streak > 0)
                                🔥 {{ __('messages.keep_streak') }}
                            @else
                                {{ __('messages.start_streak') }}
                            @endif
                        </p>
                    </div>
                    ...
                    <div class="flex flex-col text-gray-600 dark:text-[#9b9b9b] hover:dark:text-[#d6d6d6] transition-all duration-300 shadow-custom rounded-3xl p-6"
                        ...
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <span
                                ...
                                {{ __('messages.achievement') }}
                            </span>
                        </div>
                        <h3 class="text-lg font-semibold mb-2 text-gray-900 dark:text-white">{{ __('messages.completed_quizzes') }}</h3>
                        <p class="text-5xl font-bold count-animation text-gray-900 dark:text-white">{{ $totalCompletedQuizzes }}</p>
                        <p class="text-sm mt-2">
                            {{ $totalCompletedQuizzes > 0 ? __('messages.great_job') : __('messages.start_completing_quizzes') }}
                        </p>
                    </div>
                    ...
                    <div class="flex flex-col text-gray-600 dark:text-[#9b9b9b] hover:dark:text-[#d6d6d6] transition-all duration-300 shadow-custom rounded-3xl p-6"
                        ...
                        <p class="text-sm mt-2">
                            {{ $enrolledCourses->count() > 0 ? __('messages.keep_learning') : __('messages.enroll_in_course') }}
                        </p>
                    </div>
                </div>
                ...
                <div class="w-full">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <a href="{{ route('student.courses') }}"
                            ...
                            <i class="fa-solid fa-plus mr-2"></i>{{ __('messages.browse_courses') }}
                        </a>
                    </div>
                                ...
                                <div class="flex gap-2 flex-col text-gray-600 dark:text-[#9b9b9b] hover:dark:text-[#d6d6d6] transition-all duration-300 shadow-custom rounded-3xl p-6"
                                    ...
                                    <div class="mt-2">
                                        <div class="flex justify-between text-xs mb-1">
                                            <span class="text-gray-500 dark:text-gray-400">{{ __('messages.progress') }}</span>
                                            <span class="font-bold">{{ $course->completion_percentage }}%</span>
                                        </div>
                                        <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                                            <div class="bg-linear-to-r from-blue-500 to-purple-500 h-2 rounded-full transition-all duration-1000"
                                    ...
                                    <div class="flex justify-between items-center mt-4">
                                        <div class="flex gap-3 text-xs">
                                            ...
                                            <span><i
                                                    class="fa-solid fa-clipboard-question mr-1"></i>{{ $course->material->sum(fn($m) => $m->quizzes->count()) }}</span>
                                        </div>
                                        <a href="{{ route('student.courses.learn', $course->id) }}"
                                            ...
                                            <span class="relative text-base font-semibold">{{ __('messages.continue_button') }}</span>
                                        </a>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    @else
                            ...
                            <div class="flex flex-col text-gray-600 dark:text-[#9b9b9b] transition-all duration-300 shadow-custom rounded-3xl p-16 text-center"
                                ...
                                <p class="text-gray-600 dark:text-gray-400 mb-8 text-lg">
                                    {{ __('messages.no_courses_enrolled') }}
                                </p>
                                <a href="{{ route('student.courses') }}"
