...
@section('content')
    <div class="flex justify-center items-center font-balo max-w-[1000px] flex-col mx-auto">
        ...
        <section
            ...
            @endphp
            @if (!$taken)
                <form action="{{ route('student.courses.take', $course->id) }}" method="post">
                    @csrf
                    <button type="submit"
            ...
            @else
                <a href="{{ route('student.courses.learning-preview',$course->id) }}"
                    ...
                    <span
                        class="absolute inset-0 w-full h-full -mt-1 rounded-lg opacity-30 bg-linear-to-b from-transparent via-transparent to-gray-200"></span>
                    <span class="relative text-base font-semibold">Start Course</span>
                </a>
            @endif
        ...
        <section class="flex gap-5 mt-5">
            <div class="flex gap-2 justify-between w-[65%] h-max items-center text-gray-600 dark:text-[#EEEEEE] shadow-custom rounded-3xl p-4 relative overflow-hidden"
                style="--color-shadow:#9b9b9b;">
                <div class="w-[55%] p-2 ">
            ...
            <div class="flex gap-2 justify-center flex-col w-[35%] h-max items-center text-gray-600 dark:text-[#EEEEEE] shadow-custom rounded-3xl p-4"
                ...
                <p class="text-center">{{ $course->user->bio }}</p>
            </div>
        </section>
        <section class="flex flex-col w-full gap-5 mt-5">
            ...
            @forelse ($course->material as $material)
                <div class="flex gap-5 w-full">
                    ...
                    <div class="flex relative  justify-center flex-col w-full h-max items-center text-gray-600 dark:text-[#EEEEEE] shadow-custom rounded-2xl p-3"
                        style="--color-shadow:#9b9b9b;">
                        <span>{{ Str::limit($material->title, 100) }}</span>
                        <i class="fa-solid fa-video text-[#9b9b9b]  absolute text-[20px] -z-1 right-4  "></i>
                    </div>
                </div>
            @empty
